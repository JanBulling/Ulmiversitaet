...
import DepartureListItem from './DepartureListItem'; // Import the corrected component
...
import type { StopOption, Departure } from '../departure/types';
...
const UniversityDeparturesDisplay: React.FC<UniversityDeparturesDisplayProps> = ({ allStops }) => {
  ...
  const filteredStops = useMemo(() => {
    ...
    if (lowercasedSearchTerm.length === 0) return allStops;
  ...
  const groupedDepartures = useMemo(() => {
    ...
    departures.forEach(dep => {
      const key = `${dep.RouteNumber}-${dep.DepartureDirectionText}`;
      if (!groups[key]) {
        groups[key] = [];
      }
      groups[key].push(dep);
    ...
    const limitedGroups: Departure[][] = [];
    Object.keys(groups).forEach(key => {
      const sortedGroup = groups[key].sort((a, b) =>
        new Date(a.DepartureTimeScheduled).getTime() - new Date(b.DepartureTimeScheduled).getTime()
      );
      limitedGroups.push(sortedGroup.slice(0, 3)); // Limit to 3 departures per group
    });
  ...
  const renderDepartureContent = useMemo(() => {
    ...
    return (
      <ul className="space-y-2 max-h-[220px] overflow-y-auto scrollbar-gutter">
        {groupedDepartures.map((group, i) => (
          // *** PASS THE GROUP ARRAY TO DepartureListItem ***
          <DepartureListItem
            key={i}
            departures={group} // Pass the array for this group
            routeIconUrl={routeIcons[group[0]?.RouteNumber]} // Use route number from the first departure in the group
  ...
  }, [currentStopId, isLoading, groupedDepartures, error, routeIcons, currentTime, showCountdown, truncateDirectionText]);
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="w-5 h-5 flex items-center justify-center">
          {selectedStopId ? (
                ...
                <path d="M5.2 10.5C5.2 7.84905 7.34903 5.70001 10 5.70001" className="wave wave--small"/>
                <path d="M2 10.5C2 6.08172 5.58172 2.5 10 2.5" className="wave wave--big"/>
                </svg>
          ) : (
